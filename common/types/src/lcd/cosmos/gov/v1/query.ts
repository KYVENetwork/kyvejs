...
export const QueryProposalsRequest = {
  encode(message: QueryProposalsRequest, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.proposal_status !== ProposalStatus.PROPOSAL_STATUS_UNSPECIFIED) {
      writer.uint32(8).int32(proposalStatusToNumber(message.proposal_status));
    }
    if (message.voter !== "") {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          message.proposal_status = proposalStatusFromJSON(reader.int32());
          continue;
        case 2:
          if (tag !== 18) {
            break;
  ...
  fromPartial<I extends Exact<DeepPartial<QueryProposalsRequest>, I>>(object: I): QueryProposalsRequest {
    const message = createBaseQueryProposalsRequest();
    message.proposal_status = object.proposal_status ?? ProposalStatus.PROPOSAL_STATUS_UNSPECIFIED;
