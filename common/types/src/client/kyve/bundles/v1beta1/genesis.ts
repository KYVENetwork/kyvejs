...
export const GenesisState: MessageFns<GenesisState> = {
  encode(message: GenesisState, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    for (const v of message.bundle_proposal_list) {
      BundleProposal.encode(v!, writer.uint32(18).fork()).join();
    }
    for (const v of message.finalized_bundle_list) {
      FinalizedBundle.encode(v!, writer.uint32(26).fork()).join();
    }
    for (const v of message.round_robin_progress_list) {
      RoundRobinProgress.encode(v!, writer.uint32(34).fork()).join();
    }
    if (message.bundle_version_map !== undefined) {
      BundleVersionMap.encode(message.bundle_version_map, writer.uint32(42).fork()).join();
    }
    return writer;
  },
